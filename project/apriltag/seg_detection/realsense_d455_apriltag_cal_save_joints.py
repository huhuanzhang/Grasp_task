...
parser.add_argument("--require_tag_on_save", action="store_true", help="If set, only save when target tag is visible/stable (old behavior)")
...
print("\nControls:")
print("  - Press 's' (or 'p') to save ONE sample")
print("    default: save joints even if tag not visible")
print("    use --require_tag_on_save to enforce old behavior")
...
try:
    while True:
        ...
        save_pressed = (k == ord("s") or k == ord("p"))
        ...
        if save_pressed and (now - _last_save_t) > SAVE_DEBOUNCE_SEC:
            ...
            if NO_ROBOT:
                ...
                if q_deg is not None:
                    if args.require_tag_on_save and (tag_info is None):
                        print("[WARN] require_tag_on_save enabled and tag not stable/visible, skip save.")
                    else:
                        sample = {
                            "t": now,
                            "arm_profile": args.leader_profile,
                            "q_deg_left_5dof": q_deg,
                            "tag_visible": bool(tag_info is not None),
                            "tag_id": (int(tag_info["tag_id"]) if tag_info is not None else None),
                            "decision_margin": (float(tag_info["decision_margin"]) if tag_info is not None else None),
                            "hamming": (int(tag_info["hamming"]) if tag_info is not None else None),
                            "pose_err": (float(tag_info["pose_err"]) if tag_info is not None else None),
                            "T_cam_tag": (tag_info["T_cam_tag"].tolist() if tag_info is not None else None),
                        ...
                        if tag_info is not None:
                            print(f"[SAVED] WITH_TAG q_deg={['%.2f'%v for v in q_deg]}  z={tag_info['T_cam_tag'][2,3]:.3f} m  dm={tag_info['decision_margin']:.1f}")
                        else:
                            print(f"[SAVED] NO_TAG  q_deg={['%.2f'%v for v in q_deg]}")
